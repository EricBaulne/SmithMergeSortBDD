...
namespace SmithMergeSortUI.MergeSort.ForSmith
{
  using System;
  ...
  /// instance of the MergeSort class and call its Sort method.
  /// </example>
  /// <remarks>
  /// </remarks>
  ...
  {
    private const int InsertionBlockSize = 1;
    ...
    public void Sort(IList<T> list, bool ascending = true)
    {
      ...
      for (int index = 0; index < arrayList.Length; index++)
        ...
        protected void SortBlock(T[] list, T[] tempList, int beginBlock, int endBlock, int recursionDepth)
        {
            ...
            bool isSmallEnoughForInsertion = blockSize <= InsertionBlockSize;

            if (isSmallEnoughForInsertion)
            {
                if (mergeToTempList)
  ...
  public static class MergeSortExtension
  {
    ...
    {
      MergeSort<T> sorter = new MergeSort<T>(comparison);
      sorter.Sort(list, ascending);
    }
    ...
    {
      MergeSort<T> sorter = new MergeSort<T>(comparer);
      sorter.Sort(list, ascending);
    }
